...
新版已加入实作修正：荧光标签、局部过曝、下屏内受控随机放置一个蕾丝贴片（三种形态择一），无毛边、毛须和散线、珠链串联与不对称复印拼贴。用户的私人照片与成片不包含在开源包内。`assets/generated/`中的两张无人像AI底板可用于测试素材流程，实际输出应按照片改文字、布局和元素，不能所有用户都套同一张底板。
